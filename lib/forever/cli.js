...
var help = [
  ...
  '  --tailable       log files will be rolled based on maxsize and maxfiles, but in ascending',
  '                   order. The filename will always have the most recent log lines.',
  '                   The larger the appended number, the older the log file.',
  '                   This option requires maxFiles to be set, or it will be ignored.',
  '  --maxsize SIZE   Max size in bytes of the logfile, if the size is exceeded then a new',
  '                   file is created, a counter will become a suffix of the log file.',
...
var getOptions = cli.getOptions = function (file) {
  var options = {},
      ...
      configKeys = [
        ...
        'killTree', 'killSignal', 'id', 'tailable', 'maxsize', 'maxFiles', 'zippedArchive'
      ],
  ...
  function assignSpawnWith(options) {
    ...
    // The test/core/start-stop-json-array-test.js was broken due to this line below.
    // The problem is that in forever-monitor there is another check on the sourceDir that casuse a further concatenation of the    
    // script path. Since with spawnWith we define a working dir, the process start for fixtures/servers.json wouldn't work.
    // Not sure which line is to drop if the one below or 
    // https://github.com/foreverjs/forever-monitor/blob/master/lib/forever-monitor/monitor.js#L134 (probably the latter)
