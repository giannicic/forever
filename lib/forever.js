...
forever.cleanLogsSync = function (processes) {
  ...
  runningLogs = running && running.map(function (p) {
    return path.basename(p.logFile, '.log');
  });

  files.forEach(function (file) {
    if ((/\.log$/.test(file) && (!runningLogs || ((runningLogs.indexOf(file.replace(/\.log$/,"")) === -1) && (runningLogs.indexOf(file.replace(/[0-9]\.log$/,"")) === -1)))) ||
        (/\.log.gz$/.test(file) && (!runningLogs || runningLogs.indexOf(file.replace(/[0-9]\.log.gz$/,"")) === -1))) {
